...
public class Level3Button : ActivatorButton
{
    ...
    protected override IEnumerator transformButton()
    {
        ...
        Vector3 startBonus = bonusWall.transform.localPosition;
        ...
        while (timer <= 1)
        {
            if (hits >= hitsToActivate)
            {
                ...
                bonusWall.transform.localPosition = Vector3.Lerp(startBonus, startBonus + vOffset, timer);
            } else
            {
                transform.localPosition = Vector3.Lerp(startPos,
